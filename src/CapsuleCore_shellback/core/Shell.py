...
class Shell(ABC):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type:
            ...
            self.logger.info(
                f"Sesión de {self.__class__.__name__} finalizada correctamente"
            )
    ...
    def run(self, command, timeout: Optional[float] = None) -> CommandResult:
        ...
        if not shutil.which(executable):
            ...
            raise CommandNotFoundError(
                f"The executable '{executable}' was not found in PATH."
            )
        ...
        try:
            ...
            if result.is_success():
                ...
                self.logger.warning(
                    f"Failed: {executable} with code {result.return_code}"
                )
