...
def test_run_external_success(shell_stub, mock_observer):
    ...
    with patch("shutil.which", return_value="/usr/bin/ls"), \
        ...
        mock_observer.on_command_start.assert_called_once_with("ls", ["/usr/bin/ls", "-la"])
        mock_observer.on_command_result.assert_called_once()
        assert isinstance(mock_observer.on_command_result.call_args[0][0], CommandResult)
...
def test_run_external_timeout(shell_stub, mock_observer):
    ...
    with patch("shutil.which", return_value="/usr/bin/sleep"), \
         patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd=["sleep"], timeout=1.0)):
